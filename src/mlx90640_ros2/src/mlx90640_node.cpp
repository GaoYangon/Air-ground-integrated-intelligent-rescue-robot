...
private:
  ...
  // 修改：移除bus参数，使用MLX_I2C_ADDR
  void InitializeMLX90640(void)
  {
    // 初始化 I2C 通信
    MLX90640_I2CInit();

    // 读取并校准参数（使用MLX_I2C_ADDR而非bus）
    uint16_t eeMLX90640[832];
    MLX90640_DumpEE(MLX_I2C_ADDR, eeMLX90640);
    MLX90640_ExtractParameters(eeMLX90640, &params_);

    // 设置刷新速率（使用MLX_I2C_ADDR而非bus）
    MLX90640_SetRefreshRate(MLX_I2C_ADDR, 0x03);  // 8Hz
  }

  void publishThermalImage()
  {
    // 读取帧数据（使用MLX_I2C_ADDR而非i2c_bus_）
    uint16_t frame[834];
